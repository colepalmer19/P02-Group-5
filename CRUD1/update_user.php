...
$name = $email = $contact_information = $area_of_expertise = '';

if ($user_id > 0) {
    $stmt = $conn->prepare("SELECT name, email, contact_information, area_of_expertise FROM user WHERE id = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $stmt->bind_result($name, $email, $contact_information, $area_of_expertise);
    $stmt->fetch();
    $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update user details (with password hash if provided)
    if (!empty($new_password)) {
        $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("UPDATE user SET name = ?, email = ?, contact_information = ?, area_of_expertise = ?, password = ? WHERE id = ?");
        $stmt->bind_param("sssssi", $name, $email, $contact_information, $area_of_expertise, $hashed_password, $user_id);
    } else {
...
<body>
    ...
    <div class="top-bar">
        ...
        <a href="user_management.php" class="home-btn">Cancel</a>
    </div>
