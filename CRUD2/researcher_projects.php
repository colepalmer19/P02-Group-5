...
// Handle adding or assigning research projects
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $assigned_to = isset($_POST['assigned_to']) ? $_POST['assigned_to'] : []; // Ensure assigned_to is an array
    ...
    if (isset($_POST['id']) && !empty($_POST['id'])) {
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['update_success_message'] = "Project updated successfully!";
        } else {
            $_SESSION['error_message'] = "Error updating project: " . htmlspecialchars($stmt->error);
        }
    } else {
        // ✅ Creating a new project
        $stmt = $conn->prepare("INSERT INTO research_projects (title, description, funding, status, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())");
    ...
    <div class="projects-container">
        ...
        <div class="create-container">
            ...
            <?php if (isset($_SESSION['update_success_message'])): ?>
                <div class="message success"><?php echo $_SESSION['update_success_message']; unset($_SESSION['update_success_message']); ?></div>
            <?php endif; ?>
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="add-project-form">
                ...
                <div class="form-group">
                    <textarea 
                        ...
                        rows="10" 
                        required 
                        style="resize: none; display: block; width: 100%; max-width: 1160px; height: 100px; margin-bottom: 20px;"
                ...
                <tbody>
                    <?php while ($row = $ongoingProjectsQuery->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['assigned_to_names'] ?: 'Unassigned'); ?></td>
                            <td>
                                <a href='researcher_project_update.php?id=<?php echo htmlspecialchars($row['id']); ?>'>Update</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
